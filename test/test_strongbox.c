...
static void selective_readwrite_quicktest(int normal_ciphers)
{
    ...
    // Make sure the first part is still as it is supposed to be
    TEST_ASSERT_EQUAL_MEMORY(in_buffer2, out_buffer1, nugget_size * 2);
    TEST_ASSERT_EQUAL_MEMORY(in_buffer1 + nugget_size * 2, out_buffer1 + nugget_size * 2, nugget_size);
}
